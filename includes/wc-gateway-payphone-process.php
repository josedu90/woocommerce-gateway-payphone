...
class WC_Gateway_PayPhone_Process {
    ...
    private function get_payphone_args($order_id) {
        ...
        $request_params->Amount = round($order->get_total(), 2) * 100;
        $request_params->AmountWithOutTax = 0;
        $request_params->AmountWithTax = 0;
        $request_params->Tax = round($order->total_tax, 2) * 100;
        ...
        $request_params->AmountWithTax = round($request_params->AmountWithTax, 2) * 100;
        $request_params->AmountWithOutTax = round($request_params->AmountWithOutTax, 2) * 100;
